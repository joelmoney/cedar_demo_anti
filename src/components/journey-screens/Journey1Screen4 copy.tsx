...
interface Journey1Screen4Props {
  reducedMotion?: boolean;
  onNext?: () => void;
}

export function Journey1Screen4({ reducedMotion = false, onNext }: Journey1Screen4Props) {
    ...
    const dobTimer = setTimeout(() => {
      ...
      const typeInterval = setInterval(() => {
        if (index <= target.length) {
          ...
          // Start typing last name after DOB is complete
          const lastNameTimer = setTimeout(() => {
            const lastNameTarget = 'Thorne';
            let lastNameIndex = 0;
